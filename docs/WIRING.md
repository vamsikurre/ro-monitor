...
> **Amended 2026-09-14 — the fault is INTERMITTENT, and that weakens several rows of the table above.** On the following day the plant stopped on tank-full correctly, with the same wiring still landed. Every row above rests on a single observation, and a single observation of an intermittent fault proves much less than it appears to. In particular "identical behaviour with the hub powered off" may have been an unlucky sample rather than proof of the phantom path, and the interlocks that "failed" may have failed on that attempt rather than always. Treat the table as the sequence of reasoning, not as four proven eliminations.
>
> **What intermittency points at.** Two candidates, and they are not exclusive:
>
> * **A marginal connection in the hub's own leg.** Two conductors under one screw - the float's and ours - is precisely the arrangement that makes and breaks. When our wire makes contact the shunt exists and the float loses authority; when it does not, the plant behaves correctly. Every trip to the terrace disturbs that block and re-rolls the dice, which also explains why the behaviour changed after a visit.
> * **The LED sitting at its conduction threshold.** A PC817's forward voltage moves about -2 mV/degC, and this hub is on a terrace. Hot afternoon, lower Vf, the LED conducts and masks the float; cool morning, it does not. The overflow happened during the day. The hub's own SHT30 and the dashboard history can test this - correlate the failures against RO room temperature and time of day.
>
> **Neither changes the remedy, and both raise its urgency.** An interlock that works when you test it and fails when you do not cannot be validated by watching it work. It can only be fixed by removing the cause. Nobody should read "it stopped correctly today" as evidence that the tap is safe.
...
| **A ~35.3 ms echo pulse means "too close", not "far away"** | With a target inside the ~200 mm blind zone the module holds `ECHO` high for a fixed **~35.3 ms** rather than not pulsing at all. Converted naively that reads as **~604 cm**, which looks like a confident long-range measurement and is the opposite. Our `US_TIMEOUT_US` is 35 000 µs, so `pulseIn` gives up 263 µs before that pulse ends and correctly returns 0 — the firmware reports no echo where a bare sketch would report 604 cm. **A steady 604 cm from a test sketch means the sensor cannot range, usually because a hand, a cable or a bench edge is inside 20 cm.** Established 2026-09-05. |
| **A steady ~600 µs echo, once per ping, with nothing behind it — on a node, never on the hub** | `pulseIn()` on the ATmega counts loop iterations, not time. Every byte on the shared bus (polls and replies for the *other* nodes included) runs the SoftwareSerial RX interrupt for ~1 ms with interrupts off, and those iterations are never counted: a 6.4 ms echo came back as ~600 µs, fell inside the blind zone and was refused. `US_DEBUG` shows it as `d1` with **no second pulse** in the remaining window — a real near-field artifact would still have the true echo behind it. `pingOnce()` now calls `stopListening()` for the ≤35 ms of the ping; the hub's driver times with `esp_timer` and was never affected, which is why the dosing sensor read a steady 452 mm through the whole hunt. Trigger width and sensor supply were investigated first and cleared. |
