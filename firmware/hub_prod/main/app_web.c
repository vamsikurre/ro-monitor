...
static esp_err_t cal_get(httpd_req_t *req)
{
    ...
    n += snprintf(page + n, WEB_SCRATCH_BYTES - n,
        ...
        "<p><small><b>Relay 1 is wired inverted as built</b> (WIRING.md &sect;7.2): its "
        "contact is parallel with the treated water float, so energising it shorts that "
        "float out and tells the Aster the tank is NOT full &mdash; it starts the plant "
        "rather than stopping it, and on 2026-09-13 nothing stopped it again. Expect that "
        "when testing, and watch the tank. "
        "Each button energises that relay for <b>about %d s</b> and the hub "
